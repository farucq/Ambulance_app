...
import { Mail, Lock, AlertTriangle, ArrowRight, Siren } from 'lucide-react';
...
const Login = ({ setAuth }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const user = await db.verifyLogin(formData.email, formData.password);
      if (user.role !== role) {
        throw new Error(`This account is not registered as an ${role}`);
      }
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row bg-[#080808] overflow-hidden relative font-sans selection:bg-red-500/30">
      {/* LEFT SECTION: Minimal Text Branding */}
      <motion.div
        layoutId="branding-section"
        className="relative w-full md:w-1/2 h-[30vh] md:h-screen flex items-center justify-center p-12 bg-gradient-to-br from-[#0a0a0a] to-[#121212] border-r border-white/5 overflow-hidden"
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
           <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_20%_20%,rgba(220,38,38,0.05)_0%,transparent_50%)]" />
        </div>
        ...
        <div className="relative z-10 text-center md:text-left">
          ...
          >
            Authorized access for medical personnel and emergency dispatch control units only.
          </motion.p>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="relative w-full md:w-1/2 h-full flex items-center justify-center p-8 lg:p-20 bg-[#080808] overflow-hidden rounded-3xl"
      >
        ...
        >
          <div className="bg-white/[0.02] backdrop-blur-3xl rounded-[3rem] border border-white/5 p-10 lg:p-12 shadow-[0_50px_100px_-15px_rgba(0,0,0,0.5)] relative group">
             ...
             <div className="mb-12">
                <h2 className="text-2xl font-black text-white tracking-tight uppercase mb-2">ACCESS HUB</h2>
                <p className="text-slate-500 font-bold text-xs uppercase tracking-widest">Awaiting Verification</p>
             </div>
             ...
             <form onSubmit={handleLogin} className="space-y-8">
                ...
                <div className="flex justify-center gap-6 mt-8">
                  <Link to="/" className="text-[10px] text-slate-500 font-black uppercase tracking-widest hover:text-white transition-colors">
                    RETURN HOME
                  </Link>
                  <Link to="/signup" className="text-[10px] text-red-500 font-black uppercase tracking-widest hover:text-red-400 transition-colors">
                    BECOME A DRIVER
                  </Link>
                </div>
             </form>
